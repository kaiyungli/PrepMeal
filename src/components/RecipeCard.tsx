...
export default function RecipeCard({ recipe, onClick, onFavorite, className = '', imageHeightClass = 'h-48' }: RecipeCardProps) {
  ...
  return (
    <div onClick={() => { if (onClick) onClick(); }} className={`cursor-pointer bg-white rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl transition-all border border-transparent hover:border-[#C8B79B] ${className}`}>
      {/* Image */}
