// FavoriteButton - local-first optimistic UI
...
/**
 * FavoriteButton - local-first optimistic UI
 * - Maintains local state for instant click response
 * - Calls onToggle without waiting
 * - No dependency on global favorites state
 */
function FavoriteButton({ 
  ...
  // Sync with prop when it changes (not during interaction)
  React.useEffect(() => {
  ...
  const handleClick = useCallback((e: React.MouseEvent) => {
    ...
    onToggle(recipeId);
    setIsLoading(false);
  }, [localFav, onToggle, recipeId, isLoading]);
