...
export default async function handler(req, res) {
  ...
  try {
    ...
    } else if (safeSort === 'time_short' || safeSort === 'quick') {
      query = query.order('total_time_minutes', { ascending: true, nullsFirst: false }).order('id', { ascending: false });
    } else if (safeSort === 'protein_high' || safeSort === 'high_protein') {
      query = query.order('protein_g', { ascending: false, nullsFirst: false }).order('id', { ascending: false });
    } else if (safeSort === 'low_calorie') {
      query = query.order('calories_per_serving', { ascending: true, nullsFirst: false }).order('id', { ascending: false });
    } else if (safeSort === 'calories_high') {
      query = query.order('calories_per_serving', { ascending: false, nullsFirst: false }).order('id', { ascending: false });
    } else {
      query = query.order('created_at', { ascending: false }).order('id', { ascending: false });
    }
import { supabaseServer as supabase } from '@/lib/supabaseServer'
